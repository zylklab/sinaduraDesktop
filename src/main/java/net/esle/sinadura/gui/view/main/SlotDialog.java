...
public class SlotDialog extends Dialog {
	...
	public Map<String, Long> listaSlots; 
	

	/**
	 * @param parent
	 */
	public SlotDialog(Shell parent) {
		super(parent);
	}
	 ...
	 * @return
	 */
	public String open(Map<String, Long> listaSlots) {
		...
		this.sShell
				.setImage(new Image(sShell.getDisplay(), Thread.currentThread().getContextClassLoader().getResourceAsStream(ImagesUtil.SINADURA_LOGO_IMG)));
		this.sShell.setText(LanguageUtil.getLanguage().getString("slot.dialog.title"));
		...
		textoLabel.setText(LanguageUtil.getLanguage().getString("slot.dialog.message"));
		
		GridData gdTexto = new GridData();
		gdTexto.widthHint = 400;
		textoLabel.setLayoutData(gdTexto);
		
		this.slotCombo = new Combo(this.sShell, SWT.NONE | SWT.READ_ONLY);
		this.slotCombo.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		
		Set<String> names = listaSlots.keySet();
		...
		for (String string : names) 
		{
			...
			this.slotCombo.add(string);
			this.slotCombo.select(0);
		}
	...
	class BotonAceptarListener implements SelectionListener {
		...
		public void widgetSelected(SelectionEvent event) {
			...
			//slot = slots.get(slotCombo.getSelectionIndex());
			slot = listaSlots.get(slots.get(slotCombo.getSelectionIndex()))+"";
			sShell.dispose();
		}
